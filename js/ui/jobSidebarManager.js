...
function createJobIconElement(jobName, selectedJobs) {
  ...
  iconDiv.addEventListener("click", () => {
    const alreadySelected = iconDiv.classList.contains("selected");

    // Clear all other selections
    document.querySelectorAll(".job-icon.selected").forEach((el) => {
    ...
    updateFilterValue(
      "selectedJobs",
      selectedJob ? new Set([selectedJob]) : new Set()
    );
  });
...
function setupSidebarCollapseHandlers(
  ...
  sidebar.addEventListener("mouseleave", () => {
    ...
    // Collapse only if sidebar is open and at least one job is selected
    if (!isCollapsed() && filterState.selectedJobs.size > 0) {
      collapseSidebar();
    }
  });
